// CLI Demo Modal Functionality
// Showcases the CLI tool's demo feature on the website

(function() {
    'use strict';
    
    // Demo samples - same as in cli/src/demo-samples.ts
...
fn find_max(numbers: &[i32]) -> Option<i32> {
    ...
    function init() {
        ...
        const cliDemoModal = document.getElementById('cliDemoModal');
        const cliDemoModalClose = document.getElementById('cliDemoModalClose');
        const cliDemoLanguageGrid = document.getElementById('cliDemoLanguageGrid');
        
        if (!cliDemoBtn || !cliDemoModal) return;
        ...
        cliDemoBtn.addEventListener('click', function() {
            cliDemoModal.classList.add('active');
            
            // Track analytics
            if (window.polyglotAnalytics) {
                window.polyglotAnalytics.trackEvent('cli_demo_opened', {
                    source: 'cli_section'
                });
            }
        });
        ...
        // Close modal
        cliDemoModalClose.addEventListener('click', closeModal);
        cliDemoModal.addEventListener('click', function(e) {
            if (e.target === cliDemoModal) {
                closeModal();
            }
        });
        ...
        document.addEventListener('keydown', function(e) {
            if (e.key === 'Escape' && cliDemoModal.classList.contains('active')) {
                closeModal();
            }
        });
    }
    ...
    function populateLanguageGrid() {
        ...
        Object.keys(CLI_DEMO_SAMPLES).forEach(lang => {
            const sample = CLI_DEMO_SAMPLES[lang];
            const card = document.createElement('div');
            card.className = 'cli-demo-lang-card';
            card.innerHTML = `
                <div class="cli-demo-lang-name">${sample.displayName}</div>
                <div class="cli-demo-lang-desc">${sample.description}</div>
            `;
    ...
    function closeModal() {
        ...
        // Reset state
        setTimeout(() => {
            document.getElementById('cliDemoLanguageGrid').parentElement.style.display = 'block';
            document.getElementById('cliDemoPanels').style.display = 'none';
