...
async function cmdConfigureApiKey(): Promise<void> {
    ...
    const apiKey = await vscode.window.showInputBox({
        ...
        ignoreFocusOut: true,
        validateInput: val =>
            val && val.trim().length > 10 ? null : 'Key must be at least 10 characters',
    });
    if (!apiKey) return;
    ...
    vscode.window.showInformationMessage(
        `✅ Poly-Glot: ${provider.value === 'anthropic' ? 'Anthropic' : 'OpenAI'} configured!`,
    );
}
