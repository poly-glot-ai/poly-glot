...
function authHeaders(extra: Record<string, string> = {}): Record<string, string> {
    ...
    const ver: string = require('../package.json').version as string ?? '0.0.0';
 ...
 */
async function flushOfflineUsageQueue(token: string): Promise<void> {
    const queued = extContext.globalState.get<number>('pg.offlineUsageQueue', 0);
    if (queued <= 0) return;
    try {
        ...
        if (res.ok) {
            ...
            if (typeof data.used === 'number') await syncLocalCount(data.used);
        }
    ...
    try {
        ...
        if (lastKnownPlan && PRO_PLANS.includes(lastKnownPlan)) {
            ...
            vscode.window.showWarningMessage(
                '⚠️ Poly-Glot: Server unreachable — generating offline. Usage will sync when reconnected.',
            );
            return true;
        }
 ...
 */
async function requireSignUp(source: string): Promise<void> {
    ...
    if (email) {
        try {
            ...
            if (res.ok) {
                ...
                vscode.window.showInformationMessage(
                    `✅ Magic link sent! Click it in your email and VS Code will sign you in automatically — no copy-paste needed.`,
                    'Open Email',
                ...
                vscode.window.showInformationMessage(
                    `🦜 Poly-Glot: Account found for ${email}. Enter your session token to sign in.`,
                    'Enter Token',
                ).then(async (choice) => {
                    if (choice === 'Enter Token') await cmdConfigureLicenseToken();
                });
            }
        } catch {
            vscode.window.showErrorMessage(
                '🚫 Poly-Glot: Could not reach the server. Please check your connection.',
                'Try Again',
                'Sign Up on Web',
            ).then(async (choice) => {
                if (choice === 'Try Again') {
        ...
        vscode.window.showWarningMessage(
            '🦜 Poly-Glot requires a free account. Sign up in 30 seconds — no credit card needed.',
            'Sign Up Free',
...
async function checkForNewerVersion(context: vscode.ExtensionContext): Promise<void> {
    try {
        const currentVersion = context.extension.packageJSON.version as string;
        const lastChecked    = extContext.globalState.get<number>('pg.versionCheckedAt', 0);
        const now            = Date.now();

        // Only check once every 24 hours
        if (now - lastChecked < 24 * 60 * 60 * 1000) return;
        ...
        const latestVersion: string = data?.results?.[0]?.extensions?.[0]?.versions?.[0]?.version || '';
        ...
        const choice = await vscode.window.showWarningMessage(
            `🦜 Poly-Glot v${latestVersion} is available (you have v${currentVersion}). Update now to get the latest features and security fixes.`,
            'Update Now',
            'Later',
        );
...
async function maybeShowFirstRunOnboarding(): Promise<void> {
    ...
    if (!email) {
        ...
        vscode.window.showWarningMessage(
            '🦜 Poly-Glot requires a free account to generate comments. Sign up in 30 seconds.',
            'Sign Up Free',
            'I Already Have an Account',
        ).then(async (choice) => {
            if (choice === 'Sign Up Free') {
    ...
    try {
        ...
        if (res.ok) {
            ...
            vscode.window.showInformationMessage(
                `✅ Check your inbox for ${email} — click the magic link and VS Code will sign you in automatically.`,
                'Open Email',
                'Enter Token Manually',
            ).then(async choice => {
                if (choice === 'Open Email') {
                    ...
                    if (!hasKey) {
                        const apiChoice = await vscode.window.showInformationMessage(
                            '🔑 Almost there! Add your AI API key to start generating.',
                            'Configure API Key', 'Later',
                        );
                        if (apiChoice === 'Configure API Key') await cmdConfigureApiKey();
                    }
                }
            ...
            const tokenChoice2 = await vscode.window.showInformationMessage(
                `🦜 Account found for ${email}. Enter your session token to sign in.`,
                'Enter Token',
            );
            if (tokenChoice2 === 'Enter Token') {
                ...
                if (!hasKey) {
                    const apiChoice = await vscode.window.showInformationMessage(
                        '🔑 Almost there! Set up your AI API key to start generating.',
                        'Configure API Key', 'Later',
                    );
                    if (apiChoice === 'Configure API Key') await cmdConfigureApiKey();
                }
        ...
        // Network error
        vscode.window.showErrorMessage(
            '🚫 Poly-Glot: Could not reach the server. Please check your connection, then try generating to sign up.',
            'Sign Up on Web',
        ).then(async (choice) => {
 ...
 */
class PolyGlotUriHandler implements vscode.UriHandler {
    async handleUri(uri: vscode.Uri): Promise<void> {
        ...
        if (!hasKey) {
            const apiChoice = await vscode.window.showInformationMessage(
                '🔑 Almost there! Add your AI API key to start generating comments.',
                'Configure API Key',
                'Later',
            );
            ...
            vscode.window.showInformationMessage(
                '🚀 Ready! Run Poly-Glot: Generate Comments on any file to get started.',
                'Generate Now',
            ).then(async choice => {
...
export function activate(context: vscode.ExtensionContext): void {
    ...
    context.subscriptions.push(
        ...
        vscode.commands.registerCommand('polyglot.startForFree',            () => cmdStartForFree()),
...
async function cmdStartForFree(): Promise<void> {
    // If already signed in with a session token, nothing to do
    const existing = extContext.globalState.get<string>('pg.sessionToken', '').trim();
    if (existing) {
        vscode.window.showInformationMessage(
            'Poly-Glot: You already have a free account. Run "Poly-Glot: Configure License Token" to upgrade.',
            'See Pro Plans',
        ).then(a => { if (a === 'See Pro Plans') vscode.env.openExternal(vscode.Uri.parse(UPGRADE_URL)); });
        return;
    }
    ...
    await vscode.window.withProgress(
        { location: vscode.ProgressLocation.Notification, title: 'Poly-Glot: Sending magic link…', cancellable: false },
        async () => {
            try {
                ...
                if (res.ok && data.ok) {
                    ...
                    const action = await vscode.window.showInformationMessage(
                        `✅ Magic link sent to ${email.trim()}! Check your inbox, click the link, then paste your token below.`,
                        'Enter Token',
                        'Later',
                    );
                    ...
                    vscode.window.showErrorMessage(
                        `Poly-Glot: ${data.error || 'Failed to send magic link. Please try again.'}`,
                    );
                }
            } catch {
                vscode.window.showErrorMessage(
                    'Poly-Glot: Network error — check your connection and try again.',
                );
            }
        },
    );
