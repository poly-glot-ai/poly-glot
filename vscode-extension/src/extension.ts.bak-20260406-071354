            ...
            if (devRes.status === 403 || devData.limitReached) {
                ...
                if (email) {
                    try {
                        ...
                        if (r.ok) {
                            vscode.window.showInformationMessage(
                                `🦜 Magic link sent to ${email}! Click it, then come back and enter your token.`,
                                'Enter Token Now', 'Later',
                            ).then(async c => { if (c === 'Enter Token Now') await cmdConfigureLicenseToken(); });
                        } else {
                    ...
                    vscode.window.showWarningMessage(
                        `🦜 Poly-Glot: Sign up free at poly-glot.ai to keep going — 50 files/month.`,
                        'Sign Up', 'Later',
                    ).then(c => {
                        if (c === 'Sign Up') vscode.env.openExternal(vscode.Uri.parse('https://poly-glot.ai/?source=vscode-gate'));
                    });
                }
                return false;
            }
                ...
                if (devData.used === 3) {
                    ...
                    if (nudgeEmail && nudgeEmail.includes('@')) {
                        try {
                            ...
                            if (nr.ok) {
                                vscode.window.showInformationMessage(
                                    `🦜 Magic link sent to ${nudgeEmail}! Click it, then enter your token to unlock ${FREE_LIMIT} files/month.`,
                                    'Enter Token Now', 'Later',
                                ).then(async c => { if (c === 'Enter Token Now') await cmdConfigureLicenseToken(); });
                            } else {
                                await cmdConfigureLicenseToken();
...
async function maybeShowFirstRunOnboarding(): Promise<void> {
    ...
    const email = await vscode.window.showInputBox({
        ...
        ignoreFocusOut: true,
        validateInput: (v) => {
            if (!v || !v.includes('@') || !v.includes('.')) return 'Please enter a valid email address';
            return null;
        },
    });
    ...
    if (!email) {
        ...
        const later = await vscode.window.showInformationMessage(
            '🦜 Poly-Glot: Sign up free to get 50 files/month + track usage across devices.',
            'Sign Up Now',
            'Later',
        );
        if (later === 'Sign Up Now') {
            await vscode.env.openExternal(vscode.Uri.parse(
                'https://poly-glot.ai/?source=vscode-install&utm_source=vscode&utm_medium=extension&utm_campaign=onboarding'
            ));
        }
        return;
    }
    ...
    try {
        ...
        if (res.ok) {
            ...
            vscode.window.showInformationMessage(
                `🦜 Poly-Glot: Magic link sent to ${email}! Check your inbox, click the link, then come back here.`,
                'I clicked the link — enter token',
                'Later',
            ).then(async (choice) => {
            ...
            const choice = await vscode.window.showInformationMessage(
                `🦜 Poly-Glot: Already have an account? Enter your session token to activate.`,
                'Enter Token',
                'Sign Up at poly-glot.ai',
                'Later',
            );
        ...
        const choice = await vscode.window.showInformationMessage(
            '🦜 Poly-Glot: Get 50 files/month free. Sign up in 30 seconds — no password needed.',
            'Create Free Account',
            'I Already Have One',
            'Later',
        );
