 ...
 */
async function maybeShowTeamNudge() {
    const plan = extContext?.globalState.get('pg.lastKnownPlan', '');
    if (plan !== 'pro')
        return; // only solo Pro users, not team/enterprise
    const nudgeDone = extContext.globalState.get('pg.teamNudgeDone', false);
    if (nudgeDone)
        return;
    const count = getMonthlyCount();
    if (count < 5)
        return; // wait until they've seen real value
    await extContext.globalState.update('pg.teamNudgeDone', true);
    const choice = await vscode.window.showInformationMessage(`🚀 You've processed ${count} files with Poly-Glot! Using it with teammates? Team plan is $29/mo for 5 seats.`, 'See Team Plan', 'Dismiss');
...
async function showLimitReached(used, limit) {
    const choice = await vscode.window.showErrorMessage(`🚫 Free plan limit reached — ${used}/${limit} file this month. Upgrade to Pro for unlimited files.`, 'Upgrade Pro — $12/mo', 'Team Plan — $29/mo', 'Enter Token');
...
async function showUsageNudge(used, remaining) {
    ...
    if (used >= FIRST_NUDGE_AT && remaining <= 0) {
        ...
        const choice = await vscode.window.showWarningMessage(`🔴 You've used your 1 free file this month. Upgrade to keep generating.`, 'Pro — $12/mo (unlimited)', 'Team — $29/mo (5 seats)', 'Enter Token');
        ...
        const choice = await vscode.window.showWarningMessage(`⚡ Last free file remaining this month. Pro is $12/mo for unlimited.`, 'Upgrade Pro — $12/mo', 'Team Plan — $29/mo', 'Dismiss');
        if (choice === 'Upgrade Pro — $12/mo') {
            vscode.env.openExternal(vscode.Uri.parse(UPGRADE_URL));
        }
        else if (choice === 'Team Plan — $29/mo') {
...
async function cmdConfigureLicenseToken() {
    ...
    await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: 'Poly-Glot: Verifying token…', cancellable: false }, async () => {
        ...
        if (plan && PRO_PLANS.includes(plan)) {
            ...
            if (plan === 'pro') {
                const choice = await vscode.window.showInformationMessage(`✅ Poly-Glot Pro activated! All features unlocked. Using it with a team?`, 'See Team Plan — $29/mo', 'Dismiss');
